...
| 반응형 구현            | 다양한 화면 크기에서 자연스럽게 보이도록 제작         |
...
flowchart LR
    ...
    A --> A1[정보가 많아 탐색이 복잡함]
    A --> A2[전시 콘텐츠의 시각적 집중도가 약함]
    A --> A3[메뉴 구조가 한눈에 들어오지 않음]

    B --> B1[메뉴를 카테고리별로 정리]
    B --> B2[이미지 중심의 메인 화면 구성]
    B --> B3[주요 콘텐츠 진입 경로를 단순화]

    classDef problem fill:#EFE7DC,color:#4A3A2A,stroke:#C8B6A6;
    classDef solution fill:#E8D8CD,color:#5C4033,stroke:#C9A99A;

    class A,A1,A2,A3 problem;
    class B,B1,B2,B3 solution;
```
...
| Interaction | 메뉴 이동과 콘텐츠 탐색 중심의 인터랙션 구현                  |
